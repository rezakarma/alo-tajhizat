...
import { forgetPasswordClientchema } from "@/schema/index";
...
const ForgotPasswordForm = () => {
  ...
  const {
    ...
    resolver: zodResolver(forgetPasswordClientchema),
    defaultValues: {
      email: "",
    },
  });
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-sm">
        <div className="flex flex-col w-9/12 mx-auto  gap-1 mb-1  md:w-11/12 pl-1">
        ...
        <div className="flex flex-col gap-4">
          {/* <label className="block mr-2 font-medium">
            آخرین رمز عبوری را که به خاطر میاورید وارد کنید:
          </label>
          <input
            type="password"
            {...register("lastPassword")}
            className={errors.lastPassword ? errorInputClass : okInputClass}
          />
          {errors.lastPassword?.message && (
            <p className="text-red-500">{errors.lastPassword?.message}</p>
          )} */}
          <span className="mr-5 mt-7 text-primary font-medium hover:text-primaryDark transition-all">
            <Link href="/login">ورود به حساب کاربری</Link>
          </span>
          <span className="mr-5 text-primary font-medium hover:text-primaryDark transition-all">
          ...
          <Button
            ...
            isLoading={isPending}
            type="submit"
            className="bg-primary md:mr-5 md:w-8/12 rounded-full h-10 text-white font-bold text-xl drop-shadow-lg hover:bg-primaryDark transition-all duration-200"
          >
            ارسال
          </Button>
        </div>
