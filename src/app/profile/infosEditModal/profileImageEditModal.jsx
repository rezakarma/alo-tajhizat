...
const ProfileImageEditModal = (props) => {
    ...
    if (user && user.profile) {
      dispatch({ type: "SET_ID", payload: user.profile.id });
        if (isFirstRender.current) {
          ...
          dispatch({ type: "SET_PROFILE_PHOTO_KEY", payload:user.profile.profileImage});
  ...
  const setProfileImageAndOpenEditor = (image) => {
    dispatch({ type: "SET_PROFILE_PHOTO", payload: image });
  ...
  const uploadFile = async (image, folder) => {
    ...
    if (state.profilePhotoKey === "") {
      formData.append("userName", userName);
    } else {
      const key = state.profilePhotoKey.substring(
        state.profilePhotoKey.indexOf("/") + 1
      );
  ...
  const imageUpdateHandler = async () => {
    ...
    const profileImageKey = await uploadImage(
      ...
      "profile",
      "SET_PROFILE_PHOTO_KEY",
    );

    if (state.profilePhotoKey === profileImageKey) {
      reduxDispatch(
        uiActions.showNotification({
          status: "success",
    ...
    reduxDispatch(changeProfile(state.id, { profileImage: profileImageKey }))
      .then(() => {
        update();
      })
  };
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col mt-2">
          ...
          <UploadImage
            ...
            setImage={setProfileImageAndOpenEditor}
            imageDeleteHandler={profileImageDeleteHandler}
          />
          {state.editorShow === true && (
            <ImageEditor
              image={state.profilePhoto[1]}
              setImage={setProfileImage}
              setEditorShow={editorShowHandler}
            />
          )}
        </div>
